...
for (s in systems) {
  ...
  for (i in c(1:20)) {
    ...
    for (k in c(1:10)) {
      ...
      if (k == 1) {
        ...
        predicted.nn.values = predicted.nn1.values$net.result + predicted.nn.values
      }
    }
    ...
    confusion.mat <- table(test$yes,predicted.nn.values)
  ...
  write.table(format(score, scientific=FALSE),
              col.names = T, 
              row.names = F, 
              file = paste("Desktop/Neural-Network/dataset", storename, sep="/"), sep=";")
}
